...
function Fact({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div>
      <dt className="text-xs text-[#5a5a5a] dark:text-[#9aa3b2]">{label}</dt>
      <dd className="font-semibold text-[#121212] dark:text-[#eef1f6]">
        {value}
      </dd>
    </div>
  );
    ...
    <div className="flex flex-col gap-3">
      <dl className="grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3">
        {typeLabel ? (
          <div className="flex items-start gap-2">
            ...
            <div>
              <dt className="text-xs text-[#5a5a5a] dark:text-[#9aa3b2]">
                Tipo
              </dt>
              <dd className="font-semibold text-[#121212] dark:text-[#eef1f6]">
                {typeLabel}
      ...
      {animal.physical_description ? (
        <div>
          ...
          <p className="mt-0.5 text-sm leading-snug text-[#121212] dark:text-[#eef1f6]">
            {animal.physical_description}
          </p>
        </div>
      ) : null}
